  ...
  if(UNIX)
    if(CMAKE_C_COMPILER_ID MATCHES Clang)
      ...
      target_compile_options(
        ${NAME}
        PRIVATE # Use the customized `as` for LVI mitigation instead of
                # LLVM-integrated one.
                -no-integrated-as)
    elseif(CMAKE_C_COMPILER_ID MATCHES GNU)
      # For GNU, use the following switch to ensure indirect branches via
      # register only. This property is ensured by the
      # `-x86-speculative-load-hardending`, which OE enables by default, on
      # Clang.
    ...
    if(GLIBC_VERSION GREATER_EQUAL 2.27)
      # Add a linker option to tell gcc/clang wrappers to invoke customized `as`
      # and `ld`. Note that the option is for the wrappers only. Wrappers will
      # discard the option (i.e., not feeding it to the linker).
      target_link_options(${NAME} PRIVATE -link-lvi-mitigation)
    else()
      # For system with older version of GLIBC, the compilers will invoke the
      # default `ld` instead of the customized one. However, the version
      # mismatch between the customized `as` (version >= 2.32) and the older
      # version of `ld` (< 32) cause the compilation to fail with the `-g`
      # option (see https://sourceware.org/bugzilla/show_bug.cgi?id=23919).
      # Therefore, the workaround is to disable the `-g` option by using `-g0`.
